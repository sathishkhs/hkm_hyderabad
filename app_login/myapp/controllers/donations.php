...
class Donations extends MY_Controller
{
   ...
   public function group_donations(){
		$data['charitable_programs'] = $this->payments_model->view('charitable_programs');
		$data['view'] = 'donations/group_donations_list';
		$data['title'] = 'Administrator Dashboard';
		$data['page_heading'] = 'Grouped Donations List';
		$data['breadcrumb'] = 'Grouped Donations List';
		$data['scripts'] = array('assets/javascripts/donations.js');
		$this->load->view('templates/dashboard', $data);
   }

   public function group_donations_list($slug=''){
	$draw = intval($this->input->post("draw"));
	$start = intval($this->input->post("start"));
	$length = intval($this->input->post("length"));
	$payments = $this->payments_model->get_group_pagination('donations','payments');
