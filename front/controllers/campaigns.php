...
class Campaigns extends MY_Controller {
  ...
  public function index($slug) {
        if(!empty($this->input->post())){
            ...
            $this->payment_model->data['mobile_number'] = $mobile_number = $this->input->post('mobile_number');
            $this->payment_model->data['city'] = $city = $this->input->post('city');
            $this->payment_model->data['amount'] = $amount = $this->input->post('amount');
            $this->payment_model->data['pan'] = $pan = $this->input->post('pan');
            $this->payment_model->data['dob'] = $dob = !empty($this->input->post('dob')) ? $this->input->post('dob') : '0000-00-00';
            ...
            $this->payment_model->data['citizen'] = $citizen =  $this->input->post('citizen');
            ...
            $data += [
                ...
                "image"             => "https://akshayachaitanya.org/donate/assets/images/accf-fav-logo.png",
                "prefill"           => [
                    "name"              => $name,
                    "email"             => $email,
    ...
    public function pay()
    {
       $data = $this->data;
        ...
        $json = json_encode($data);
        $data['view_path'] = $this->class_name . '/donate';
        $this->load->view('templates/campaigns/campaign_home', $data);
    ...
    public function save_payment($insert_id,$table_name)
    {
        ...
        $this->payment_model->data['order_id'] =$order_id =  $this->input->post('razorpay_payment_id');
